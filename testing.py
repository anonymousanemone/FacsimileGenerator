...
def simple_shape_from_shading(img_gray):
...
def process_pipeline(uploaded_paths, options, output_folder):
    if options.get("stitch") and len(uploaded_paths) > 1:
        base_img = stitch_images(uploaded_paths)
    else:
    ...
    if options.get("shape_from_shading"):
        sfs = simple_shape_from_shading(gray)
        gray = cv2.addWeighted(gray, 0.6, sfs, 0.4, 0)
...
if __name__ == "__main__":
    ...
    # img = cv2.imread("./sample_img/sample.jpg", cv2.IMREAD_GRAYSCALE) / 255.0
    # mask = watershed_background_segment(img)

    # cv2.imshow("mask", mask)
    # cv2.imwrite("mask.png", mask * 255)

    # img = cv2.imread("./sample_img/sample.jpg")
